...
describe('Endpoint', () => {
    ...
    describe('list', () => {
        ...
        it('should append params object as the query string', () => {
            ...
            let scope = nock('http://example.com')
                ...
                .query({key: 'value'})
                .reply(200);
            let request = endpoint.list({key: 'value'}).end(() => {
                scope.done();
                done();
            });
    ...
    describe('retrieve', () => {
        it('should make a GET request to the object url', () => {
            let endpoint = new Endpoint('http://example.com/endpoint');
            let scope = nock('http://example.com').get('/endpoint/obj').reply(200);
            let request = endpoint.retrieve('obj').end(() => {
                scope.done();
                done();
            });
        });
    ...
    describe('create', () => {
        it('should make a POST request to the endpoint url', () => {
            let endpoint = new Endpoint('http://example.com/endpoint');
            let scope = nock('http://example.com').post('/endpoint').reply(200);
            let request = endpoint.create().end(() => {
                scope.done();
                done();
            });
        });
        
        it('should send the object conf in the request body', () => {
            let endpoint = new Endpoint('http://example.com/endpoint');
            let scope = nock('http://example.com')
                .post('/endpoint', {id: 1})
                .reply(200);
            let request = endpoint.create({id: 1}).end(() => {
                scope.done();
                done();
            });
        });
    ...
    describe('update', () => {
        it('should make a PUT request to the object url', () => {
            let endpoint = new Endpoint('http://example.com/endpoint');
            let scope = nock('http://example.com').put('/endpoint/obj').reply(200);
            let request = endpoint.update({}, 'obj').end(() => {
                scope.done();
                done();
            });
        });
        ...
        it('should send the object conf in the request body', () => {
            ...
            let scope = nock('http://example.com')
                ...
                .reply(200);
            let request = endpoint.update({id: 1}, 'obj').end(() => {
                scope.done();
                done();
            });
        });
